...
import { BugIcon, CameraIcon, CloudUploadIcon, XCircleIcon, BarChartIcon } from './common/IconComponents';
...
const PlantAnalysis: React.FC<PlantAnalysisProps> = ({ texts, currentLanguage }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-gray-100 dark:bg-gray-900 text-gray-800 dark:text-gray-100 font-poppins bg-leaf-pattern">
            <header className="p-4 text-center border-b border-gray-200 dark:border-gray-700 non-printable bg-white/80 dark:bg-gray-900/80 backdrop-blur-sm">
                    ...
                    {view === 'upload' && (
                        <motion.div key="upload" className="w-full h-full flex flex-col items-center justify-center p-8" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }}>
                            <div className="text-center">
                                <BugIcon className="w-16 h-16 text-green-500 mb-4 inline-block animate-leaf-float" />
                            </div>
